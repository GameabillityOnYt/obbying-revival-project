...
var cached_character_materials: Array[StandardMaterial3D] = []
var last_alpha: float = -1.0
var _step_query: PhysicsShapeQueryParameters3D
var _step_box_shape: BoxShape3D
var _step_hit_info: KinematicCollision3D
# ==========================================
# Computed variables
# ==========================================
var rotation_locked: bool: 
	get(): return cam.mode == cam.CameraMode.FIRSTPERSON or GameManager.shiftlocked
...
func _ready() -> void:
	reset()
	cache_character_materials()
	_init_step_climbing_cache()
...
func _physics_process(delta: float) -> void:
	...
	if GameManager.nfToggle and $CollisionShape3D.disabled:
		...
		return # bypasses implemented physics

	# physics handling
	_evaluate_truss_collisions(delta)
...
func _handle_noclip_movement(delta: float) -> void:
	...
	var fly_dir : Vector3 = (cam_basis.x.normalized() * input_dir.x) + (-cam_basis.z.normalized() * input_dir.y)
...
func _handle_standard_movement(delta: float) -> void:
	...
	if is_climbing:
		_process_climbing_velocity(delta, input_dir, right)
	else:
		_process_ground_air_velocity(delta, direction)
...
func _evaluate_truss_collisions(delta: float) -> void:
	if jump_lock > 0.0:
		return
	...
	if active_ray:
		...
		var forward_check = Vector3(-sin(cam.yaw), 0, -cos(cam.yaw)).normalized()
		var right_check = Vector3(cos(cam.yaw), 0, -sin(cam.yaw)).normalized()
...
func _apply_environmental_forces(delta: float) -> void:
	if step_visual_offset != 0.0:
		...
		if player:
			player.position.y += (step_visual_offset - old_offset)

	if not is_on_floor() and not is_climbing and not $CollisionShape3D.disabled:
		velocity += get_gravity() * delta

	if truss_timer < 0.1 and not truss_used:
		coyote_timer = coyote_time if is_leg_near_ground() else 0.0
		if is_leg_near_ground(): truss_used = true
	elif is_on_floor():
...
func _process_climbing_velocity(delta: float, input_dir: Vector2, right: Vector3) -> void:
	...
	set_climb_anim(climb_input == 0, "Up" if climb_input > 0 else "Down" if climb_input < 0 else "Idle")

func _process_ground_air_velocity(delta: float, direction: Vector3) -> void:
...
func _init_step_climbing_cache() -> void:
	_step_query = PhysicsShapeQueryParameters3D.new()
	_step_box_shape = BoxShape3D.new()
	_step_box_shape.size = Vector3(0.4, 2.4, 0.2)
	_step_query.shape = _step_box_shape
	_step_query.exclude = [get_rid()] 
	
	_step_hit_info = KinematicCollision3D.new()
...
func _step_climbing() -> void:
	...
	if test_move(global_transform, step_displacement, _step_hit_info):
		_handle_step_up(step_displacement, _step_hit_info)
	elif not is_on_floor():
		var space_state = get_world_3d().direct_space_state
		_step_query.transform = global_transform.translated(step_displacement)
		
		var results = space_state.intersect_shape(_step_query , 1) # limit it to 1 for performance reasons
		if !results.is_empty():
			_handle_step_up(step_displacement, _step_hit_info)
	...
	if is_on_floor():
		...
		if test_move(forward_tgt, down_sweep, _step_hit_info):
			var drop_dist = _step_hit_info.get_travel().y
...
func _handle_step_up(step_displacement: Vector3, hit_info: KinematicCollision3D) -> void:
	...
	var upward_transform = global_transform.translated(Vector3(0.0, max_possible_step, 0.0))
	
	if test_move(upward_transform, step_displacement):
		return
		
	var forward_tgt = upward_transform.translated(step_displacement)
	var drop_sweep = Vector3(0.0, -max_possible_step, 0.0)
	var ground_hit = _step_hit_info
		
	if test_move(forward_tgt, drop_sweep, ground_hit):
		var drop_dist = ground_hit.get_travel().y
		var final_step_height = max_possible_step + drop_dist
...
# ==========================================
# Visuals and animation handling
# ==========================================
func cache_character_materials():
	cached_character_materials.clear()
	_gather_character_recursive($Character)

func set_char_transparency(alpha: float) -> void:
	if is_equal_approx(alpha, last_alpha):
		return
	last_alpha = alpha
...
func _gather_character_recursive(node: Node):
	if node is MeshInstance3D:
		...
		if material is StandardMaterial3D:
			cached_character_materials.append(material)
			
	for child in node.get_children():
		_gather_character_recursive(child)

func set_climb_anim(pause: bool, dir: String):
	if pause:
		playerAnims.speed_scale = 0.0
	else:
		if dir == "Up": playerAnims.speed_scale = 1.0
		elif dir == "Down": playerAnims.speed_scale = -1.0
		else: playerAnims.speed_scale = 0.0
...
func update_state():
	if is_climbing:
		...
		if velocity.y > 0: State = states.Jumping
...
func reset():
	if $CollisionShape3D: $CollisionShape3D.disabled = false
	if cam: cam.freecam_active = false
	...
	if spawn != null:
		...
		if spawn.has_meta("saved_velocity"):
			...
			if velocity.length() > 0.1: knockback_timer = 0.1 
		else:
